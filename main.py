...
from datetime import datetime   # To date calculation

# Folder containing Python scripts to run
folder_path = r"./sources"
    ...
    if filename.endswith('.py'):
        ...
        try:
            ...
            process = subprocess.run(
                # python as a command or python3
                ['python3', file_path],
                capture_output=True,
                text=True
            )
...
categories_with_meta = {
    ...
    "usedSoftware": [
        "SOCMET (Semantic Open Data Category Extraction Tool — umbrella framework)",
...
# Save combined output to single file for month
final_output_file = os.path.join("./output", 'combined_statistics.json')
with open(final_output_file, 'w', encoding='utf-8') as f:
    json.dump(combined_statistics, f, ensure_ascii=False, indent=2)

# Save combined output to single file for dashboard
final_output_file_dashboard = os.path.join("./dashboard/public/output", 'combined_statistics.json')
with open(final_output_file_dashboard , 'w', encoding='utf-8') as f:
    json.dump(combined_statistics, f, ensure_ascii=False, indent=2)
